...
def rrf(min_source_lists: list[list[MinimalSource]], k: int) -> list[MinimalSource]:
    if len(min_source_lists) == 1:
        return min_source_lists[0][:k]
...
def generate_hyde(queries: list[str]) -> list[str]:
    cache_path = "data/processed/hyde_answers"
    ...
    for query in queries:
        if query in data:
            hyde_answers.append(data[query])
            continue

        prompt_messages = [
            {
                "role": "system",
                "content": "Answer the given question in 1 short sentence. It's okay to guess. /no_think",
            },
            {
            "role": "user",
                "content": f"Question: {query}",
        ...
        hyde_ans = response.choices[0].message.content
        hyde_answers.append(hyde_ans)
